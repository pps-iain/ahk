...
;===============================================================================
;===============================================================================
::pacasso.card::pacasso.card_v
::chopin.card::chopin.card_v
::selct::select
...
:*:{nf}::{noformat} {noformat}
:*:{code}:: {code}
:*:{sql}::{code:sql} {code}
:*:{red}::{color:#FF0000} {color}
:*:{blue}::{color:#003366} {color}
::cahse::chase
...
::promotioni::promotion
::Tehrefore ::Therefore
::obvciously ::obviously
::gullable::gullible
...
; Field Mismatches
::txn-created::txn_created
::txn-status::txn_status
::crd-status::crd_status
::crd_additonal-1:: crd_additonal-1
::cst-id::cst_id
::cst-id::cst_id
::bch-id::bch_id
::acc-id::acc_id
::txn-acc_id::txn_acc_id
::txn-acc-id::txn-acc-id
::TAS-id::TAS_id
::txn-id::txn_id
...
::seelect::select
::seleect::select
::select 8::select * from
:*:sel.pac.::select * from pacasso.
:*:sel.cho.::select * from chopin.
;?> expands for db
:*:?>PC::SELECT * FROM PACASSO.CARD_V WHERE
:*:?>PRC::SELECT * FROM PACASSO.PRODUCT_CLASS WHERE
:*:?>T::SELECT * FROM PACASSO.TRANSACTION
:*:?*P::select * from pacasso.
:*:?*C::select * from chopin.
::?*C::select * from chopin
; ??PC Expands db
:*:??D::SELECT * FROM `nPACASSO.CARD_V WHERE
:*:??V::SELECT * FROM `nPACASSO.CARD_V WHERE
:*:??T::SELECT * FROM `nPACASSO.TRANSACTION
:*:??X::SELECT * FROM `nPACASSO.TRANSACTION WHERE TXN_REF_INT='
:*:??P::select * from `npacasso.
:*:??R::SELECT * FROM `nPCSREPORT.
:*:??L::SELECT * FROM `nPACASSO.LOG_REQ_RESP.
:*:??Q::SELECT * FROM `nCHOPIN.LOG_REQ_RESP.
:*:??S::SELECT * FROM `nSECURITY.
:*:??C::select * from `nchopin.
:*:??O::select * from `nCONFMGR.
:*:??u::select * from `npacasso.customer where lower(cst_name) like '%&name%';
:*:??A::select * from `npacasso.account where 
:*:??B::select * from `npacasso.account_balance where

; Db System Expands
:*:///V::SELECT * FROM PACASSO.CARD_V `nORDER BY CRD_CREATED DESC;
:*:///N::ALTER SESSION `n set NLS_date_format='DD-MON-YY HH24:MI:SS';
:*:///T::SELECT * FROM PACASSO.TRANSACTION `nORDER BY TXN_CREATED DESC;
:*:///PRC::SELECT * FROM `nPACASSO.PRODUCT_CLASS `nWHERE
:*:///PLOG::SELECT * FROM `nPACASSO.LOG_REQ_RESP `nWHERE
:*:///CLOG::SELECT * FROM `nCHOPIN.LOG_REQ_RESP `nWHERE
:*:///ABA::SELECT * FROM `nPACASSO.ACCOUNT_BALANCE `nWHERE
:*:///C::SELECT * FROM `nPACASSO.CARD_V `nWHERE
:*:///K::SELECT * FROM `nPACASSO.TASK
;==========================================
:*:crd#::crd_ext_seqno=
:*:#crd::crd_ext_seqno=
::chopin-batch-files::chopin_batch_files
::chopin-batch::chopin_batch
...
::crd_ext_seq_no::crd_ext_seqno



; ============ JIRA Related ==================
; writes an empty panel  Return at the end stops the Sendraw command from executing further

:*::Pa::
sendraw, {panel}`n`n{panel}
send, {up 1}
Return
...
; sql code block
:*::sql::
sendraw,{code:sql}`n`n{code}
send, {up 1}
Return

; noformat block
:*::nof::
sendraw,{noformat}`n`n{noformat}
send, {UP 1}
Return
...
;code:xml block
:*::xml::
sendraw,{code:xml}`n`n{code}
send, {UP 1}
Return

; empty code block
:*::code::
sendraw,{code}`n`n{code}
send, {UP 1}
Return